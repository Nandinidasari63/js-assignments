...
function occurrences(string, subString) {
  ...
  while (index < stringLen && subStringlen > 0) {
    let trackerForSubStringIndex = index;
    while (string[trackerForSubStringIndex] === subString[x] && x < subStringlen) {
      x++;
      trackerForSubStringIndex++;
...
function message(string, subString, exceptedOutput) {
  ...
  console.log(emoji, expect, exceptedOutput, "but it actually was", actualOutput);
}
